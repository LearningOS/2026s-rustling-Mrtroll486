...
impl Graph for UndirectedGraph {
    ...
    fn add_edge(&mut self, edge: (&str, &str, i32)) {
        ...
        if let Some(item) = self.adjacency_table_mutable().get_mut(edge.0).unwrap().iter_mut().find(|(name, _)| name == edge.1) {
            item.1 = edge.2;            
        } else {
            self.adjacency_table.get_mut(edge.0).unwrap().push((edge.1.to_string(), edge.2));
        }

        if let Some(item) = self.adjacency_table_mutable().get_mut(edge.1).unwrap().iter_mut().find(|(name, _)| name == edge.0) {
            item.1 = edge.2;            
        } else {
